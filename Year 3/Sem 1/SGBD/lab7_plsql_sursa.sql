...
select location_id from dept_*** order by 1;

-- rulare pentru 1000, 1700
SELECT *
FROM error_***;
...
BEGIN  
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('comanda SELECT 1 nu returneaza nimic');
  END;
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('comanda SELECT 2 nu returneaza nimic');
  END;
...
DBMS_OUTPUT.PUT_LINE(v_job);
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('comanda SELECT 3 nu returneaza nimic');
  END;
