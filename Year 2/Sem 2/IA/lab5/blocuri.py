        ...
        if h is not None:
            ...
            for i in range(problema.N):
                for j in range(len(info[i])):
                    # h = nr de blocuri care nu sunt la locul lor
                    if len(scop[i]) <= j:
                        h += 1
...
class Problema:
    def __init__(self):
        ...
        self.nod_start = Nod([['a'], ['c', 'b'], ['d']], float('inf'))  # de tip Nod
        self.nod_scop = [['b', 'c'], [], ['a', 'd']]  # doar info (fara h)
...
class NodParcurgere:
    ...
    def expandeaza(self):
        ...
        for i in range(self.problema.N):
            # pentru fiecare dintre stive scoatem elementul din varf
            # si il punem in fiecare dintre celelalte stive
            aux = copy.deepcopy(self.nod_graf.info)
            if not aux[i]:
                continue
            c = aux[i].pop()
            for j in range(self.problema.N):
                if i != j:
                    succ = copy.deepcopy(aux)
                    succ[j].append(c)  # unul dintre succesori
