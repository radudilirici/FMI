...
class Problema:
    def __init__(self):
        ...
        self.nod_start = Nod([[0, 0], [3, 3], 1], float('inf'))  # misionari, canibali pe malul stang / drept
        self.nod_scop = [[3, 3], [0, 0], 0]                      # al 3-lea elemt e malul pe care e barca
...
class NodParcurgere:
    ...
    def expandeaza(self):
        ...
        for m in range(min(info[mal][0], self.problema.M) + 1):
            # m e nr de misionari
            # pentru fiecare nr de misionari luam toate nr de canibali <= m (care inca incap in barca_
            # sau <= M daca m e 0 si vedem daca putem merge pe celalalt mal
            if m == 0:
                c_max = self.problema.M
            else:
                c_max = min(self.problema.M - m, m, info[mal][1])
            for c in range(c_max + 1):
                if m + c == 0:
                    continue
                succ = [[], [], 1 - mal]
                succ[mal] = [info[mal][0] - m, info[mal][1] - c]
                succ[1 - mal] = [info[1 - mal][0] + m, info[1 - mal][1] + c]
