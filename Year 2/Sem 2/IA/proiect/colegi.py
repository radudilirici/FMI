...
nr_euristica = 1
l_fisiere = [('input_1.txt', 'output_1.txt'),
...
class Nod:
    def __init__(self, info, h=None, scop=None, N=None):
        ...
        if h is not None:
            ...
            # calculam distanta pana la coleg in cazul in care nu ar exista restrictiile cu
            ...
            else:  # nr_euristica == 3 -- euristica gresita
                if rand_nod != rand_scop:
                    self.h += abs(N - 2 - info[0]) * 2  # il obligam sa mearga pana pe penultimul rand mereu
                    i_nou = N - 2
                else:
                    i_nou = info[0]

            # am calculat distanta pana in spatele clasei
            # calculam distanta ramasa pana la coleg (manhattan sau euclidiana pentru a doua euristica)
            if nr_euristica in [1, 3]:
                self.h += abs(scop[0] - i_nou) + abs(scop[1] - info[1])
            else:
                self.h += sqrt(scop[0] ** 2 + i_nou ** 2) + sqrt(scop[1] ** 2 + info[1] ** 2)
...
class Problema:
    def __init__(self):
        ...
        with open(l_fisiere[nr_fisier][0], 'r') as fin:
            rand = next(fin).split()
...
def a_star():
    ...
    l_open = [rad_arbore]  # open va contine elemente de tip NodParcurgere
    closed = []  # closed va contine elemente de tip NodParcurgere

    gasit = False
    t_inainte = int(round(time() * 1000))
    while len(l_open) > 0:
        print(str_info_noduri(l_open))  # afisam lista open
        nod_curent = l_open.pop(0)  # scoatem primul element din lista open
        ...
        for (nod_succesor, cost_succesor) in l_succesori:
            ...
            if not nod_curent.contine_in_drum(nod_succesor):
                ...
                if nod_parcg_vechi is not None:  # "nod_succesor" e in closed
                    ...
                    nod_parcg_vechi = in_lista(l_open, nod_succesor)
                ...
                if nod_nou:
                    ...
                    i = 0
                    while i < len(l_open):
                        if l_open[i].f < nod_nou.f:
                            i += 1
                        else:
                            while i < len(l_open) and l_open[i].f == nod_nou.f and l_open[i].g > nod_nou.g:
                                i += 1
                            break

                    l_open.insert(i, nod_nou)

    t_dupa = int(round(time() * 1000))
    with open(l_fisiere[nr_fisier][1], 'w') as fout:
        fout.write("Calculatorul a \"gandit\" timp de " + str(t_dupa - t_inainte) + " milisecunde.\n\n")
        if gasit:
            fout.write("Drum de cost minim: \n" + afisare_sol(nod_curent.drum_arbore()))
        else:
            fout.write("Nu exista drum de la nodul start la nodul scop")


if __name__ == "__main__":

    while True:
        aux = int(input("Fisierul de input:\n"
                        "1 - nu are solutie\n"
                        "2 - startul e nod final\n"
                        "3 - drum scurt si nu merge pe euristica 3\n"
                        "4 - drum lung\n"))
        if aux in range(1, 5):
            nr_fisier = aux - 1
            break
    ...
    while True:
        aux = int(input("Euristica (1 si 2 sunt corecte, 3 e gresita):\n"))
        if aux in range(1, 4):
            nr_euristica = aux
            break
        print("Input incorect")
